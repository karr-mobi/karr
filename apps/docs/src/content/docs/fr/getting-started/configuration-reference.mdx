...
Au minimum, vous devez définir l'`APP_URL` pour que votre instance fonctionne correctement.
...
| `pkce`         |             | Boolean | Paramètres de requête supplémentaires à inclure dans les demandes d'autorisation.                          |
