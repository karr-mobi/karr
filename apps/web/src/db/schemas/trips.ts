...
export const tripsView = pgView("trips_view").as((qb) => {
    return qb
        .select({
            ...
            accountProvider: profileTable.accountProvider,
            accountRemoteId: profileTable.accountRemoteId
        })
        .from(tripsTable)
        .leftJoin(profileTable, eq(tripsTable.driver, profileTable.id))
})

export const TripSchema = createSelectSchema(tripsView).omit({
    accountProvider: true,
    accountRemoteId: true
})
